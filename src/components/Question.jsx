...
const Question = ({ number, answers, question, isAdmin = false }) => {
  ...
  const deleteQuestionHandler = (ques) => {
    swal({
      title: "Сигурен ли си?",
      text: "След като бъде изтрит, няма да можете да възстановите този въпрос!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
        deleteQuestion(dispatch, ques.quesId, token).then((data) => {
          if (data.type === questionsConstants.DELETE_QUESTION_SUCCESS) {
            swal(
              "Въпросът е изтрит!",
              `Въпрос с номер ${ques.quesId}, е успешно изтрит.`,
              "success"
            );
          } else {
            swal(
              "Въпросът не е изтрит!",
              `Въпрос с номер ${ques.quesId} НЕ е успешно изтрит.`,
              "error"
            );
          }
        });

    ...
    if (isTimesUp) {
      submitQuiz(dispatch, userId, quizId, answers, token).then((data) => {
        if (data.type === quizResultConstants.ADD_QUIZ_RESULT_SUCCESS) {
          swal(
            "Тестът е изпратен!",
            `Постигнахте ${data.payload.totalObtainedMarks} точки в ${quizTitle} теста.`,
            "success"
          );
          return navigate("/quizResults");
        } else {
          swal(
            "Тестът НЕ е изпратен!",
            `${quizTitle} Все още е активен. Можете да промените вашите отговори.`,
            "info"
          );
          return navigate("/quizResults");
        }
      ...
      swal({
        title: "Сигурен ли си?",
        text: "Веднъж изпратени, няма да можете да променяте отговорите си!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((willSubmit) => {
